...
{
    BF_set_key(InputK, &tbc2->key); 
    md5_state_t md5s;
    md5_init(&md5s);
    md5_append(&md5s, InputK, 0x40),
        md5_finish(&md5s, tbc2->md5);
...
void Tabula_Encrypt2(TABULACRYPT2 *tbc2, unsigned int *PacketData, unsigned int Len)
{
    ...
    unsigned int X = SwitchEndianInt(((unsigned int*)tbc2->md5)[0]);
    unsigned int Y = SwitchEndianInt(((unsigned int*)tbc2->md5)[1]);
    for(unsigned int i=0; i<(Len/8); i++){
        unsigned int* a2 = PacketData+i*2;
        a2[0] = SwitchEndianInt(a2[0]);
        a2[1] = SwitchEndianInt(a2[1]);
        a2[0] ^= X;
        a2[1] ^= Y;
        BF_encrypt(a2, &tbc2->key);
        X = a2[0];
        Y = a2[1];
        a2[0] = SwitchEndianInt(a2[0]);
        a2[1] = SwitchEndianInt(a2[1]);
...
void Tabula_Decrypt2(TABULACRYPT2 *tbc2, unsigned int *PacketData, unsigned int Len)
{
    ...
    for(unsigned int i=0; i<(Len/8); i++){
        ...
        //Store new XOR
        unsigned int X2 = a2[0];
        unsigned int Y2 = a2[1];
        BF_decrypt(a2,&tbc2->key);
